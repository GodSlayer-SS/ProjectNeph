...
import type { AdmissionItem, HistoryItem, MemoryItem, PaletteRunResponse, StartupDiagnostics } from "../lib/bindings";
import { invokeTyped } from "../lib/ipc";

// Re-export types that panel components need to import.
export type { AdmissionItem, HistoryItem, MemoryItem, StartupDiagnostics } from "../lib/bindings";
...
export type SettingsSection = "general" | "ai" | "privacy" | "memory" | "folders" | "about";
...
type PaletteState = {
  ...
  refreshAdmissionQueue: () => Promise<void>;
  keepAdmission: (id: number) => Promise<void>;
  discardAdmission: (id: number) => Promise<void>;
...
export const usePaletteStore = create<PaletteState>((set, get) => ({
  ...
  refreshAdmissionQueue: async () => {
    const admissionItems = await invokeTyped("get_admission_queue");
    set({ admissionItems });
  },
  keepAdmission: async (id) => {
    await invokeTyped("keep_admission", { id });
    await get().refreshAdmissionQueue();
    await get().refreshMemory();
  },
  discardAdmission: async (id) => {
    await invokeTyped("discard_admission", { id });
    await get().refreshAdmissionQueue();
  },
